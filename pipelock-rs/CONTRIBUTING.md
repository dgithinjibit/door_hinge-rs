# Contributing to pipelock-rs

Thanks for considering a contribution. `pipelock-rs` is an experimental
...
- **No fail-open paths.** Any error during scan or forward must collapse to
  ...
  `pipelock-proxy/tests/proxy_e2e.rs` and the per-layer tests in
  `pipelock-scanner`.
- **DLP runs before SSRF/DNS.** Reordering these would make DNS itself a
  data-exfiltration channel. The order is documented in
  `pipelock-scanner/src/lib.rs` and verified by the scanner tests.
