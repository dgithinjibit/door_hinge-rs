...
pub(crate) fn drop_all_capabilities() -> Result<(), SandboxError> {
    ...
    if let Err(e) = drop_capability_sets() {
        ...
        if let SandboxError::Io(ref io_err) = e {
            if io_err.raw_os_error() == Some(libc::EPERM) {
                ...
                tracing::debug!(target: "pipelock.sandbox", "capset returned EPERM (acceptable in user namespace)");
                return Ok(());
            }
        }
        return Err(e);
    ...
    tracing::debug!(target: "pipelock.sandbox", "All capabilities dropped");
    Ok(())
}
