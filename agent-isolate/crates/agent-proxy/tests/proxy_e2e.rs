...
//!
//! Boots an upstream HTTP server on a random port, then a `agent-proxy`
...
//! 2. The blocked request returns 403 + `x-agent-blocked: 1`.
...
use agent_recorder::{verify_chain, Recorder};
...
async fn spawn_proxy(cfg: Config, auditor: Auditor) -> std::net::SocketAddr {
    let listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
    let addr = listener.local_addr().unwrap();
    let state = agent_proxy::build_state(cfg, auditor);
    tokio::spawn(async move {
        let _ = agent_proxy::serve_with_listener(listener, Arc::clone(&state)).await;
    });
    addr
}
